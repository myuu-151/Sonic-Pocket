...
// Standalone adaptation of MAME's TLCS-900 core for the Sonic Pocket recomp.
// Upstream: mamedev/mame src/devices/cpu/tlcs900/tlcs900.h (see reference/mame/COMMIT).
// Changes: the MAME device framework is replaced by a small bus interface, and all
// state is public so recompiled code can share it with the interpreter.
#ifndef SPR_CPU_TLCS900_H
#define SPR_CPU_TLCS900_H

#pragma once

#include "mame_compat.h"
...
// Memory interface used by the CPU core. Addresses are 24-bit.
class tlcs900_bus
{
public:
	virtual ~tlcs900_bus() = default;
	virtual uint8_t read_byte(offs_t addr) = 0;
	virtual void write_byte(offs_t addr, uint8_t data) = 0;
};

class tlcs900_device
{
public:
	explicit tlcs900_device(tlcs900_bus &bus) : m_bus(&bus) {}
	virtual ~tlcs900_device() = default;

	void device_start();
	virtual void device_reset();

	// Runs instructions until at least `cycles` cycles have elapsed; returns cycles used.
	...
	// emulated the code at `pc` itself (and updated PC/registers), so no
	// instruction is fetched this step. Used for the high-level BIOS.
	virtual bool trap_hook(offs_t pc) { (void)pc; return false; }
	virtual void execute_set_input(int inputnum, int state) = 0;

	tlcs900_bus *m_bus;
	int m_am8_16 = 0;

	uint8_t RDMEM(offs_t addr) { return m_bus->read_byte( addr & 0xffffff ); }
	uint16_t RDMEMW(offs_t addr) { return RDMEM( addr ) | ( RDMEM( addr + 1 ) << 8 ); }
	uint32_t RDMEML(offs_t addr) { return RDMEMW( addr ) | ( RDMEMW( addr + 2 ) << 16 ); }
	void WRMEM(offs_t addr, uint8_t data) { m_bus->write_byte( addr & 0xffffff, data ); }
	void WRMEMW(offs_t addr,uint16_t data) { WRMEM( addr, data & 0xff ); WRMEM( addr + 1, data >> 8 ); }
	void WRMEML(offs_t addr,uint32_t data) { WRMEMW( addr, data & 0xffff ); WRMEMW( addr + 2, data >> 16 ); }
...
{
public:
	explicit tlcs900h_device(tlcs900_bus &bus);
